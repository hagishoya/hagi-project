...
# [ 入力画像のチャネル変換 ]
# PNG 等アルファチャネルがある場合は 4チャネル -> 3チャネルへ変換
if no_makeup.shape[2] == 4:
    print('[before] shape: {}'.format(no_makeup.shape))
    no_makeup = no_makeup[:,:,:3]
    print('[after] shape: {}'.format(no_makeup.shape))

X_img = np.expand_dims(preprocess(no_makeup), 0)
makeups = glob.glob(os.path.join('imgs', 'makeup', '*.png'))
...
for i in range(len(makeups)):
    ...
    if makeup.shape[2] == 4:
        print('[before] shape: {}'.format(makeup.shape))
        makeup = makeup[:,:,:3]
        print('[after] shape: {}'.format(makeup.shape))
    ...
    imsave('result-{}.jpg'.format(i), Xs_[0])
